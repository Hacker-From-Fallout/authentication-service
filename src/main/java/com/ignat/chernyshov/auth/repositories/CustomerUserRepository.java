...
public interface CustomerUserRepository extends JpaRepository<CustomerUser, Long>, JpaSpecificationExecutor<CustomerUser> { 
    ...
    @Modifying
    @Query(value = "UPDATE customer_users u SET first_name = :first_name WHERE u.id = :id", nativeQuery = true)
    void updateFirstName(@Param("id") Long id, @Param("first_name") String firstName);

    @Modifying
    @Query(value = "UPDATE customer_users u SET last_name = :last_name WHERE u.id = :id", nativeQuery = true)
