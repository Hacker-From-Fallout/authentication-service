...
public interface SellerUserRepository extends JpaRepository<SellerUser, Long>, JpaSpecificationExecutor<SellerUser> {
    ...
    @Modifying
    @Query(value = "UPDATE seller_users u SET email = :email WHERE u.id = :id", nativeQuery = true)
    void updateEmail(@Param("id") Long id, @Param("email") String email);

    @Modifying
    @Query(value = "UPDATE seller_users u SET phone_number = :phone_number WHERE u.id = :id", nativeQuery = true)
    void updatePhoneNumber(@Param("id") Long id, @Param("phone_number") String phoneNumber);

    @Modifying
    @Query(value = "UPDATE seller_users u SET hash_password = :hash_password WHERE u.id = :id", nativeQuery = true)
    void updateHashPassword(@Param("id") Long id, @Param("hash_password") String hashPassword);

    @Modifying
    @Query(value = "UPDATE seller_users u SET last_login_date = :last_login_date WHERE u.id= :id", nativeQuery = true)
    void updateLastLoginDate(@Param("id") Long id, @Param("last_login_date") LocalDateTime lastLoginDate);

    @Modifying
    @Query(value = "UPDATE seller_users u SET account_non_expired= :account_non_expired WHERE u.id= :id", nativeQuery = true)
    void updateAccountNonExpired(@Param("id") Long id, @Param("account_non_expired") boolean accountNonExpired);

    @Modifying
    @Query(value = "UPDATE seller_users u SET account_non_locked= :account_non_locked WHERE u.id= :id", nativeQuery = true)
    void updateAccountNonLocked(@Param("id") Long id, @Param("account_non_locked") boolean accountNonLocked);

    @Modifying
    @Query(value = "UPDATE seller_users u SET credentials_non_expired= :credentials_non_expired WHERE u.id= :id", nativeQuery = true)
    void updateCredentialsNonExpired(@Param("id") Long id, @Param("credentials_non_expired") boolean credentialsNonExpired);

    @Modifying
    @Query(value = "UPDATE seller_users u SET enabled= :enabled WHERE u.id= :id", nativeQuery = true)
    void updateEnabled(@Param("id") Long id, @Param("enabled") boolean enabled);

    @Modifying
    @Query(value = 
        "INSERT INTO seller_user_authorities (user_id, authorities) VALUES (:user_id, :authority) " +
    ...
    @Modifying
    @Query(value = "DELETE FROM seller_user_authorities WHERE user_id= :user_id AND authorities= :authority", nativeQuery = true)
    void removeAuthority(@Param("user_id") Long userId, @Param("authority") String authority);

    @Modifying
    @Query(value = "DELETE FROM seller_user_authorities WHERE user_id= :user_id", nativeQuery = true)
    void deleteAllAuthorities(@Param("user_id") Long userId);

    @Modifying
    @Query(value = 
        "INSERT INTO seller_user_roles (user_id, roles) VALUES (:user_id, :role) " +
        "ON CONFLICT (user_id, roles) DO NOTHING", nativeQuery = true)
    void addRole(@Param("user_id") Long userId, @Param("role") String role);

    @Modifying
    @Query(value = "DELETE FROM seller_user_roles WHERE user_id= :user_id AND roles= :role", nativeQuery = true)
    void removeRole(@Param("user_id") Long userId, @Param("role") String role);

    @Modifying
    @Query(value = "DELETE FROM seller_user_roles WHERE user_id= :user_id", nativeQuery = true)
